...
with st.form("quiz_form"):
    ...
    # 加上 index=None，網頁打開時就會是空白、不預選
    q1 = st.radio("1. 當週末放假時，你通常更傾向於？", ["跟朋友出去聚會唱歌（E）", "待在家裡看劇休息（I）"], index=None)
    q2 = st.radio("2. 面對未來的規劃，你通常是？", ["隨遇而安，走一步算一步（P）", "做詳細的計畫表按部就班（J）"], index=None)
    q3 = st.radio("3. 當朋友跟你訴苦時，你的第一反應通常是？", ["幫他分析原因並給出建議（T）", "先安慰他的情緒表示理解（F）"], index=None)
    q4 = st.radio("4. 在觀察周遭事物時，你更在意的是？", ["眼前的現實與具體細節（S）", "未來的可能性與直覺靈感（N）"], index=None)
